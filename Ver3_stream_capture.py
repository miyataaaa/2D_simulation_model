...
class Initial_Topograpy:
    ...
    def select_initial_topograpy(self, **kwargs):
        ...
        elif initTp_function == "natureQGIS":
            x, init_z = self.natureQGIS(**kwargs)
...
#         print(z_terraces)
    ...
    def natureQGIS(self, **kwargs):
        ...
        """
        Inputpath = kwargs['xzExfile']
        if type(Inputpath) == list:
            Inputpath = Inputpath[0]
        df = pd.read_excel(Inputpath, 'for simulation', index_col=None)
...
#         from IPython.core.debugger import Pdb; Pdb().set_trace()
    ...
    def whole_uniform_uplift(self, x, z, z_LeftBoundary, z_RightBoundary):
                                 
        """計算対象領域全体が同じ隆起速度で隆起する全体一様隆起を表現する関数。ver3は境界条件に本流のシミュレーション結果を使用する。"""
        ...
        # 左右非対称クラスのための一時的操作
        z_add_uplift = np.hstack((z_LeftBoundary, np.hstack((z_add_uplift_ex, z_RightBoundary))))
...
#             z_add_uplift[j] = z[j] + (self.U * self.dt)
        ...
        return z_add_uplift            
    #@profile    
    def do_computing(self, x, init_z, iterNum, z_LeftBoundary, z_RightBoundary, uplift="uniform"):
...
#             print(i)
    ...
    #             from IPython.core.debugger import Pdb; Pdb().set_trace()
                if len(zlr) == 0:
                    ...
                    if uplift == "uniform":
                        z_array[i] = self.whole_uniform_uplift(x, z_only_erosion, z_LeftBoundary[i], z_RightBoundary[i])
                    elif uplift == "tilting":
