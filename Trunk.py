...
class Simulator:
    def __init__(self, InputTopography, **kwargs):
...
#             print(f"i : {i}")
            if i == 0:
                ...
                z_allstep[i] = self.Add_U(self.stream_power_model(x, z_allstep[i-1], ContA, i), x)
...
#             print(f"Erate: {Erate}")
    ...
    def Add_U(self, z, x):
        if self.UpliftModel == "uniform_uplift":
            z = self.uniform_uplift(z)
        elif self.UpliftModel == "tilting_uplift":
            z = self.tilting_uplift(z, x)
        else:
            print("incorrect uplift model!")
            sys.exit()
            
        return z
            
    def uniform_uplift(self, z):
        U = self.U
        dt = self.dt
...
class Multi_Simulator:
    ...
    def __init__(self, initTp1, initTp2, simulation_param1, simulation_param2):
        self.instanceList = [Simulator(initTp1, **simulation_param1), Simulator(initTp2, **simulation_param2)]
...
#     MultiSimulator = Multi_Simulator(initTp_yasuda, initTp_namura, simulation_yasuda, simulation_namura)
#     MultiSimulator.main()
