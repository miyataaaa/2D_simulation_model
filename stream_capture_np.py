...
#         from IPython.core.debugger import Pdb; Pdb().set_trace()
    ...
    def tilting_uplift(self, x, z):
        ...
        """空間勾配を持った隆起速度。分水界左側の河口側境界が最も遅く、右側の河口境界が最も速い"""
        
        dt = self.dt
        
        ex_boundary_x = x[1:-1]
        ex_boundary_z = z[1:-1]
        tilt_U = 0.014 * ex_boundary_x
...
#         from IPython.core.debugger import Pdb; Pdb().set_trace()
    ...
    def whole_uniform_uplift(self, x, z):
        ...
        ex_boundary_z = z[1:-1]
        z_add_uplift = ex_boundary_z + (U * dt)
...
#             z_add_uplift[j] = z[j] + (self.U * self.dt)
    ...
    def do_computing(self, x, init_z, iterNum):
...
#         fig.savefig("img_g1_5000yr_kb2.8e-12_location.png")
    ...
    def plot_erosion_rate(self, x, l_erosion_rate, r_erosion_rate, r_dsp, h_dsp, stream_cp, fname, iterNum):
...
#         from IPython.core.debugger import Pdb; Pdb().set_trace()
        ...
        if r_dsp == 0 and h_dsp == 0:
            ...
            for i in range(1, len(l_erosion_rate), int(nmax/20)):
                ...
                plt.plot(x[:boundary_l], l_erosion_rate[i-1], color = cm.jet(i/nmax), label='t={:.3f} yr'.format(T + i/365)) #marker='o', ms=3, label='t={} yr'.format(i/365)
                plt.plot(x[boundary_r:], r_erosion_rate[i-1], color = cm.jet(i/nmax))
            ...
            for i in range(1, len(l_erosion_rate[:h_dsp+1]), int(nmax/20)):
                if i < r_dsp:
                    ...
                    if l_erosion_rate[r_dsp] == 0:
                        ...
                        plt.plot(x[boundary_r:], r_erosion_rate[i-1], color = cm.jet(i/nmax), label='t={:.3f} yr'.format(T + i/365))
                    else:
                        # 右側の河川が消失した場合
                        # hillslopeの左端のインデックス
                        boundary_l = len(l_erosion_rate[i-1])
