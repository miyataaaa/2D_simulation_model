...
class DEM_Uplift_maker_mobilable_Leftboundary:
    ...
    def Add_U(self, z):
        ...
        U = self.U   
        boundary_U = U[0]/20
        return np.hstack((z[0]+boundary_U*dt, z[1:-1] + U[1:-1]*dt, z[-1]))
...
#             initTP.MultipulPitRm()
    ...
    def _Make_UmakerInst(self):
        ...
        for param_set, UmakerName, initTPinst in zip(self.ParamList, self.UmodelList, self.InitTpInsts):
            ...
            if UmakerName == "DEM_Uplift_maker_mobilable_Leftboundary":
                self.UmakerInsts.append(DEM_Uplift_maker_mobilable_Leftboundary(initTPinst, dt=param_set['dt']))
...
if __name__ == "__main__":
    ...
    simulation_1 = {
        ...
        "DatasetNum": 60, #総計算時間を決める数。具体的にはnamx*iterNumだけ計算する。メモリの都合上、このように総計算時間を分割した。
        "n" : 1,  # ストリームパワーモデルにおける急峻度指数 n
        "m" : 0.5, #　ストリームパワーモデルにおける流域面積指数 m
        "kb" : 1e-5, # 侵食係数 [m^0.4/s] 2.6e-15(n2, m1, U0.5), 2.4e-12(n0.8, m0.3, U0.5), 2.6e-15(n1.95, m1, U0.5) 3.3e-13(n=1, m=0.5), 1.2e-5(n=1, m=0.5, yrスケール)
        "ka" : 8.6,      
        "a" : 1.67,
        "TrunkName" : "Fault10000-24000X0.45U", 
        "dirpath" : r"G:\miyata\Simulation Model\result\Truk\Double\Fault\10000&24000m\fault_to_liner\Remake",
    }
    ...
    simulation_2 = {
        ...
        "ka" : 8.6,      
        "a" : 1.67,
        "TrunkName" : "Fault10000-24000X0.53U", 
        "dirpath" : r"G:\miyata\Simulation Model\result\Truk\Double\Fault\10000&24000m\fault_to_liner\Remake",
    }
    ...
    simulation_3 = {
        ...
        "ka" : 8.6,      
        "a" : 1.67,
        "TrunkName" : "Fault10000-24000X0.60U", 
        "dirpath" : r"G:\miyata\Simulation Model\result\Truk\Double\Fault\10000&24000m\fault_to_liner\Remake",
    }
    ...
    simulation_4 = {
        ...
        "ka" : 8.6,      
        "a" : 1.67,
        "TrunkName" : "Fault10000-24000X0.68U", 
        "dirpath" : r"G:\miyata\Simulation Model\result\Truk\Double\Fault\10000&24000m\fault_to_liner\Remake",
    }
    
    dirpath = r"G:\miyata\Simulation Model\result\Truk\Double\Fault\10000&24000m\fault_to_liner\Remake"
    ExcelList = [r"Yasuda_10000-24000m±0.45mmyr_Fault.xlsx", r"Yasuda_10000-24000m±0.53mmyr_Fault.xlsx", r"Yasuda_10000-24000m±0.60mmyr_Fault.xlsx", r"Yasuda_10000-24000m±0.68mmyr_Fault.xlsx"]
...
#     #単一河川実行
#     dirpath = r"E:\研究関連\simulation\Trunk\DoubleTrunk\Tilting\AnimatonSample"
#     fname_namura = r"Namura_1.00mmyrTilting.xlsx"
...
#         "dt" : 10, #1タイムステップ当たりの時間 [yr]
#         "nmax": 1000, #時間積分の繰り返し数
#         "DatasetNum": 50, #総計算時間を決める数。具体的にはnamx*iterNumだけ計算する。メモリの都合上、このように総計算時間を分割した。
#         "n" : 1,  # ストリームパワーモデルにおける急峻度指数 n
#         "m" : 0.5, #　ストリームパワーモデルにおける流域面積指数 m
#         "kb" : 1e-5, # 侵食係数 [m^0.4/s] 2.6e-15(n2, m1, U0.5), 2.4e-12(n0.8, m0.3, U0.5), 2.6e-15(n1.95, m1, U0.5) 3.3e-13(n=1, m=0.5), 1.2e-5(n=1, m=0.5, yrスケール)
#         "ka" : 8.6,      
...
#     fname_namura = r"Namura_0.01X2U.xlsx"
#     initTp_namura = InputTopography(dirpath, fname_namura, U_flag=True)
    # initTp_namura.MultipulPitRm()
